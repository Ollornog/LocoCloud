...
| Vaultwarden Login nach Registration scheitert | `SIGNUPS_ALLOWED` als Docker-Env-Var blockiert Registration auch für eingeladene User und kann per Admin-Config-API NICHT ueberschrieben werden. Fix: `SIGNUPS_ALLOWED` NICHT als Env-Var setzen (aus `env.j2` entfernt). `vw-credentials.py` steuert per Admin-Config-API: temporaer `signups_allowed=true` → registrieren → `signups_allowed=false`. |
