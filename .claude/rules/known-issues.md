...
| Watchtower REAKTIVIERT | Watchtower war wegen Silent Breaking Changes entfernt (Netbird v0.65 Vorfall). Jetzt re-enabled für automatische Patch-Updates (nightly 02:30, alle Container). Sicher mit gepinnten Major-Tags (`nextcloud:29`) — nur Patches werden gezogen. Major-Updates weiterhin manuell via `update-customer.yml`. Deployment via `setup-security.yml`. |
...
| Paperless delete_document nur Owner | Auch mit `delete_document` Group-Permission zeigt UI den Delete-Button nur dem Owner (Design-Decision, GitHub #5521). Workaround: `owner=None` + Workflow `remove_all_owners=True` + post-consume Script setzt `delete_document` via django-guardian. Konfiguriert über `paperless_group_sync_enabled` + `paperless_permission_groups`. |
| Paperless Saved Views nicht teilbar | Kein natives View-Sharing (GitHub #427, #8286). Views müssen pro User dupliziert werden. `onboard-user.yml` kopiert Views automatisch von `paperless_views_template_user`. User muss vorher mindestens einmal per SSO eingeloggt sein. |
| Paperless Gruppen-Sync braucht LLDAP | Paperless hat kein OIDC Group Claim Mapping. Gruppen werden via `sync-groups.yml` direkt aus LLDAP → Django-Gruppen synchronisiert. Reusable Query: `roles/lldap/tasks/query-groups.yml`. |
| OIDC groups Scope für alle Apps | Alle OIDC-Clients fordern `groups` Scope an. PocketID mit LDAP-Backend liefert LLDAP-Gruppen als `groups` Claim. Apps mit nativem Group Mapping (BookStack, Planka, Grafana) nutzen den Claim direkt. Apps ohne (Paperless) brauchen custom Sync. |
