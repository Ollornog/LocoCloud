...
| Paperless delete_document nur Owner | Auch mit `delete_document` Group-Permission zeigt UI den Delete-Button nur dem Owner (Design-Decision, GitHub #5521). Workaround: `owner=None` + Workflow `remove_all_owners=True` + post-consume Script setzt `delete_document` via django-guardian. Konfiguriert über `paperless_group_permissions_enabled` + `paperless_company_group`. |
