...
#
# Usage: bash scripts/new-customer.sh <kunde_id> "<kunde_name>" "<kunde_domain>" <variante>
# Example: bash scripts/new-customer.sh abc001 "Firma ABC GmbH" "firma-abc.de" hybrid
...
# --- Argument parsing ---
if [ $# -lt 4 ]; then
    echo "Usage: $0 <kunde_id> <kunde_name> <kunde_domain> <variante>"
    echo "  variante: cloud_only | hybrid | lokal_only"
    echo ""
    echo "Example: $0 abc001 \"Firma ABC GmbH\" \"firma-abc.de\" hybrid"
    exit 1
fi
...
# --- Validate variante ---
if [[ ! "$VARIANTE" =~ ^(cloud_only|hybrid|lokal_only)$ ]]; then
    echo "ERROR: Invalid variante '$VARIANTE'. Must be: cloud_only, hybrid, lokal_only"
    exit 1
fi
...
# Contains: Netbird keys, Proxmox tokens, server IPs

vault_master_ssh_pubkey: ""
vault_netbird_key_online: ""
vault_netbird_key_lokal: ""
vault_proxmox_token: ""
vault_proxmox_netbird_ip: ""
vault_hetzner_netbird_ip: ""
vault_gateway_netbird_ip: ""
vault_apps_netbird_ip: ""
VAULT
...
echo ""
echo "Next steps:"
echo "  1. Edit: ${TARGET_DIR}/group_vars/all.yml"
echo "     - Add SSH public keys"
echo "     - Configure apps_enabled"
echo "     - Configure kunden_users"
echo "  2. Edit: ${TARGET_DIR}/group_vars/vault.yml"
echo "     - Fill in Netbird keys, IPs, tokens"
echo "  3. Encrypt: ansible-vault encrypt ${TARGET_DIR}/group_vars/vault.yml"
echo "  4. Deploy: ansible-playbook playbooks/onboard-customer.yml -i ${TARGET_DIR}/"
