...
case "$TLS_MODE" in
  ...
  cert_sync)
    ...
    echo ""
    echo "# 3. Caddy-Eintraege fuer Admin-Subdomains"
    echo "#    Falls Subdomains bereits existieren: bestehende Bloecke ersetzen."
    echo "#    Falls nicht: an die Caddyfile anhaengen."
    echo "#    Caddy holt automatisch LE-Certs fuer diese Domains."
    if [[ "$CS_CADDY" == "docker" ]]; then
      echo "#    ACHTUNG: Caddyfile liegt auf dem Host bei: ${CS_CADDYFILE}"
      echo "cat >> ${CS_CADDYFILE} << 'CADDYEOF'"
    else
