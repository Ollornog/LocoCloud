...
case "$TLS_MODE" in
  ...
  cert_sync)
    ...
    echo "# Erkennt automatisch Docker-Volume oder systemd-Pfad"
    ...
    echo ""
    echo "# Cert-Pfad ermitteln: Docker-Volume oder systemd"
    echo "CADDY_CERTS=\"\""
    echo "if docker ps --format '{{.Names}}' 2>/dev/null | grep -q '^caddy\$'; then"
    echo "    # Docker: Volume-Mountpoint auf dem Host finden"
    ...
    echo "        fi"
    echo "    done"
    echo "    # Fallback: alle Docker-Volumes nach Caddy-Certs durchsuchen"
