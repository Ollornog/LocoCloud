...
case $TLS_MODE_NUM in
  ...
  2) TLS_MODE="cert_sync"
     ...
     ask_default "SSH-User auf dem Cert-Server" CERT_SYNC_USER "certsync"
     ask "Oeffentliche IP des Cert-Servers (fuer DNS-Hinweis):" CERT_SYNC_PUBLIC_IP
     echo ""
     echo "Wie laeuft Caddy auf dem Cert-Server?"
     echo "  d) Docker-Container (z.B. /opt/stacks/caddy/)"
     echo "  s) Systemd-Service (/etc/caddy/Caddyfile)"
     ask_default "Caddy-Modus auf Cert-Server (d/s)" CERT_CADDY_MODE "d"
     if [[ "$CERT_CADDY_MODE" =~ ^[dD]$ ]]; then
       CERT_CADDY_MODE="docker"
       ask_default "Caddyfile Host-Pfad auf dem Cert-Server" CERT_CADDYFILE_PATH "/opt/stacks/caddy/Caddyfile"
     else
       CERT_CADDY_MODE="systemd"
       CERT_CADDYFILE_PATH="/etc/caddy/Caddyfile"
     fi
     ;;
  3) TLS_MODE="dns"
     echo ""
...
case "$TLS_MODE" in
  ...
  cert_sync)
    # Read the cert-sync public key (generated by Ansible in Phase 12)
    CERT_SYNC_PUBKEY=""
    if [ -f /root/.ssh/cert-sync-key.pub ]; then
      CERT_SYNC_PUBKEY=$(cat /root/.ssh/cert-sync-key.pub)
    fi
    CS_USER="${CERT_SYNC_USER:-certsync}"
    CS_HOST="${CERT_SYNC_HOST:-<CERT-SERVER-IP>}"
    CS_PUBLIC_IP="${CERT_SYNC_PUBLIC_IP:-<CERT-SERVER-PUBLIC-IP>}"
    CS_CADDY="${CERT_CADDY_MODE:-docker}"
    CS_CADDYFILE="${CERT_CADDYFILE_PATH:-/opt/stacks/caddy/Caddyfile}"

    echo -e "${BOLD}  ${STEP}. Cert-Sync einrichten (TLS: cert_sync)${NC}"
    echo ""
    echo -e "  ${BOLD}a) DNS-Eintraege (oeffentlich, auf den Cert-Server zeigend):${NC}"
    echo ""
    echo "     id.${ADMIN_DOMAIN}      -> A ${CS_PUBLIC_IP}"
    ...
    if [[ "$NETBIRD_ENABLED" =~ ^[jJyY]$ ]]; then
      ...
      echo ""
      echo "     id.${ADMIN_DOMAIN}      -> ${NETBIRD_IP:-<MASTER-NETBIRD-IP>}"
    ...
    echo ""
    echo "     Caddy-Modus: ${CS_CADDY}"
    if [[ "$CS_CADDY" == "docker" ]]; then
      echo "     Caddyfile:   ${CS_CADDYFILE}"
    fi
    echo "     Die folgenden Befehle auf dem Cert-Server per SSH ausfuehren."
    echo "     Sie richten User, Caddy-Eintraege, Export-Script und SSH-Zugriff ein."
    echo ""
    echo -e "${YELLOW}--- Schnitt: Befehle fuer den Cert-Server ---${NC}"
    echo ""
    echo "# 1. User fuer Cert-Sync anlegen"
    echo "sudo useradd -r -m -d /home/${CS_USER} -s /bin/bash ${CS_USER}"
    echo "sudo mkdir -p /home/${CS_USER}/.ssh"
    echo "sudo chmod 700 /home/${CS_USER}/.ssh"
    echo "sudo chown ${CS_USER}:${CS_USER} /home/${CS_USER}/.ssh"
    echo ""
    echo "# 2. rsync installieren (falls nicht vorhanden)"
    echo "sudo apt install -y rsync"
    echo ""
    echo "# 3. Caddy-Eintraege fuer Admin-Subdomains (idempotent)"
    echo "#    Entfernt bestehende Admin-Bloecke und fuegt den aktuellen hinzu."
    echo "#    Caddy holt automatisch LE-Certs fuer diese Domains."
    if [[ "$CS_CADDY" == "docker" ]]; then
      echo "#    ACHTUNG: Caddyfile liegt auf dem Host bei: ${CS_CADDYFILE}"
      echo "CADDYFILE=\"${CS_CADDYFILE}\""
    else
      echo "CADDYFILE=\"/etc/caddy/Caddyfile\""
    fi
    echo ""
    echo "# Bestehende Admin-Cert-Bloecke entfernen (falls vorhanden)"
    echo "# Entfernt alles zwischen '# --- LocoCloud Admin Cert-Domains ---' und der naechsten Leerzeile nach '}'"
    echo "if grep -q 'LocoCloud Admin Cert-Domains' \"\${CADDYFILE}\"; then"
    echo "    sed -i '/# --- LocoCloud Admin Cert-Domains ---/,/^}/d' \"\${CADDYFILE}\""
    echo "    echo '[INFO] Bestehenden Admin-Cert-Block entfernt.'"
    echo "fi"
    echo ""
    echo "# Auch einzelne *.admin.*-Bloecke entfernen die nicht im Marker-Format sind"
    echo "# (z.B. aus frueheren manuellen Setups)"
    echo "for sub in id vault deploy grafana nocodb auth baserow; do"
    echo "    if grep -q \"\${sub}.${ADMIN_DOMAIN}\" \"\${CADDYFILE}\"; then"
    echo "        sed -i \"/^\${sub}\\..*${ADMIN_DOMAIN//./\\.}.*{/,/^}/d\" \"\${CADDYFILE}\""
    echo "        echo \"[INFO] Alten Block fuer \${sub}.${ADMIN_DOMAIN} entfernt.\""
    echo "    fi"
    echo "done"
    echo ""
    echo "# Neuen Block anhaengen"
    echo "cat >> \"\${CADDYFILE}\" << 'CADDYEOF'"
    echo ""
    ...
    echo "CADDYEOF"
    echo ""
    echo "# Caddy neuladen damit Certs geholt werden"
    if [[ "$CS_CADDY" == "docker" ]]; then
      echo "# Docker-Caddy: IMMER restart (nie reload — Inode-Problem bei bind mounts)"
      echo "docker restart caddy"
    else
      echo "sudo systemctl reload caddy"
    fi
    echo ""
    echo "# 4. Export-Verzeichnis + Script erstellen"
    echo "#    Das Script erkennt automatisch ob Caddy als Docker oder systemd laeuft"
    echo "#    und findet den Cert-Pfad dynamisch."
    echo "sudo mkdir -p /opt/certs-export /opt/scripts"
    echo "cat << 'EXPORTEOF' | sudo tee /opt/scripts/admin-cert-export.sh > /dev/null"
    echo "#!/bin/bash"
    echo "# Exportiert Caddy-Zertifikate fuer LocoCloud Admin-Domains"
    echo "# Erkennt automatisch Docker Bind-Mount, Docker-Volume oder systemd-Pfad"
    echo "set -euo pipefail"
    echo ""
    echo "EXPORT_DIR=\"/opt/certs-export\""
    echo "ADMIN_DOMAIN=\"${ADMIN_DOMAIN}\""
    echo "ACME_DIR=\"caddy/certificates/acme-v02.api.letsencrypt.org-directory\""
    echo ""
    echo "# Cert-Pfad ermitteln: Docker (Bind-Mount/Volume) oder systemd"
    echo "CADDY_CERTS=\"\""
    echo "if docker ps --format '{{.Names}}' 2>/dev/null | grep -q '^caddy\$'; then"
    echo "    # 1. Bind-Mount: /data im Container auf Host-Pfad gemappt"
    echo "    BIND_SRC=\$(docker inspect caddy --format '{{range .Mounts}}{{if eq .Destination \"/data\"}}{{.Source}}{{end}}{{end}}' 2>/dev/null || true)"
    echo "    if [ -n \"\${BIND_SRC}\" ] && [ -d \"\${BIND_SRC}/\${ACME_DIR}\" ]; then"
    echo "        CADDY_CERTS=\"\${BIND_SRC}/\${ACME_DIR}\""
    echo "    fi"
    echo "    # 2. Named Volume"
    echo "    if [ -z \"\${CADDY_CERTS}\" ]; then"
    echo "        for vol in caddy_caddy_data caddy_data; do"
    echo "            MP=\$(docker volume inspect \"\${vol}\" --format '{{.Mountpoint}}' 2>/dev/null || true)"
    echo "            if [ -n \"\${MP}\" ] && [ -d \"\${MP}/\${ACME_DIR}\" ]; then"
    echo "                CADDY_CERTS=\"\${MP}/\${ACME_DIR}\""
    echo "                break"
    echo "            fi"
    echo "        done"
    echo "    fi"
    echo "    # 3. Fallback: alle Docker-Volumes nach Caddy-Certs durchsuchen"
    echo "    if [ -z \"\${CADDY_CERTS}\" ]; then"
    echo "        for MP in /var/lib/docker/volumes/*/; do"
    echo "            if [ -d \"\${MP}_data/\${ACME_DIR}\" ]; then"
    echo "                CADDY_CERTS=\"\${MP}_data/\${ACME_DIR}\""
    echo "                break"
    echo "            fi"
    echo "        done"
    echo "    fi"
    echo "elif systemctl is-active caddy &>/dev/null; then"
    echo "    # Systemd-Caddy: Standard-Pfad"
    echo "    CADDY_CERTS=\"/var/lib/caddy/.local/share/caddy/\${ACME_DIR}\""
    echo "fi"
    echo ""
    echo "if [ -z \"\${CADDY_CERTS}\" ] || [ ! -d \"\${CADDY_CERTS}\" ]; then"
    echo "    echo \"[ERROR] Caddy-Cert-Pfad nicht gefunden. Docker-Volume oder systemd pruefen.\" >&2"
    echo "    exit 1"
    echo "fi"
    echo ""
    echo "echo \"[\$(date -Is)] Cert-Export aus: \${CADDY_CERTS}\""
    echo ""
    echo "for sub in id vault deploy grafana nocodb; do"
    echo "    domain=\"\${sub}.\${ADMIN_DOMAIN}\""
    echo "    src=\"\${CADDY_CERTS}/\${domain}\""
    echo "    if [ -d \"\${src}\" ]; then"
    echo "        cp \"\${src}/\${domain}.crt\" \"\${EXPORT_DIR}/\${sub}.admin-fullchain.pem\""
    echo "        cp \"\${src}/\${domain}.key\" \"\${EXPORT_DIR}/\${sub}.admin-privkey.pem\""
    echo "        echo \"  \${domain} -> OK\""
    echo "    else"
    echo "        echo \"  \${domain} -> NICHT GEFUNDEN (noch kein Cert?)\" >&2"
    echo "    fi"
    echo "done"
    echo ""
    echo "chown -R ${CS_USER}:${CS_USER} \${EXPORT_DIR}"
    echo "chmod 600 \${EXPORT_DIR}/*.pem 2>/dev/null || true"
    echo "echo \"[\$(date -Is)] Export fertig.\""
    echo "EXPORTEOF"
    echo "sudo chmod +x /opt/scripts/admin-cert-export.sh"
    echo ""
    echo "# Einmal ausfuehren (kann 1-2 Min dauern bis Caddy alle Certs hat)"
    echo "sudo /opt/scripts/admin-cert-export.sh"
    echo ""
    echo "# Cron: taeglich um 03:30 (vor dem Master-Pull um 04:10)"
    echo "(sudo crontab -l 2>/dev/null; echo '30 3 * * * /opt/scripts/admin-cert-export.sh >> /var/log/cert-export.log 2>&1') | sudo crontab -"
    echo ""
    echo "# 5. SSH-Zugriff: Restricted rsync fuer den LocoCloud-Master"
    echo "cat << 'SSHEOF' | sudo tee /opt/scripts/admin-cert-sync-server.sh > /dev/null"
    echo "#!/bin/bash"
    echo "if [[ \"\$SSH_ORIGINAL_COMMAND\" == rsync\\ --server\\ --sender\\ *\\ /opt/certs-export/* ]]; then"
    echo "    \$SSH_ORIGINAL_COMMAND"
    echo "else"
    echo "    echo \"Rejected: \$SSH_ORIGINAL_COMMAND\" >&2"
    echo "    exit 1"
    echo "fi"
    echo "SSHEOF"
    echo "sudo chmod +x /opt/scripts/admin-cert-sync-server.sh"
    echo ""
    echo "# 6. SSH Public Key vom Master eintragen (restricted!)"
    echo "#    NIEMALS ssh-copy-id verwenden — das erlaubt vollen SSH-Zugang!"
    if [ -n "$CERT_SYNC_PUBKEY" ]; then
      echo "echo 'command=\"/opt/scripts/admin-cert-sync-server.sh\",no-port-forwarding,no-x11-forwarding,no-agent-forwarding,no-pty ${CERT_SYNC_PUBKEY}' | sudo tee -a /home/${CS_USER}/.ssh/authorized_keys > /dev/null"
    else
      echo "echo 'command=\"/opt/scripts/admin-cert-sync-server.sh\",no-port-forwarding,no-x11-forwarding,no-agent-forwarding,no-pty <PUBLIC-KEY>' | sudo tee -a /home/${CS_USER}/.ssh/authorized_keys > /dev/null"
      echo "# Public Key vom Master: cat /root/.ssh/cert-sync-key.pub"
    fi
    echo "sudo chmod 600 /home/${CS_USER}/.ssh/authorized_keys"
    echo "sudo chown ${CS_USER}:${CS_USER} /home/${CS_USER}/.ssh/authorized_keys"
    echo ""
    echo -e "${YELLOW}--- Ende: Befehle fuer den Cert-Server ---${NC}"
    echo ""
    if [ -n "$CERT_SYNC_PUBKEY" ]; then
      echo -e "  ${BOLD}SSH Public Key (bereits generiert):${NC}"
      echo "     ${CERT_SYNC_PUBKEY}"
      echo ""
    fi
    echo "     Nach Einrichtung: Cert-Sync vom Master testen:"
    echo "        /opt/scripts/cert-sync.sh"
    echo ""
    STEP=$((STEP + 1))
    ;;
